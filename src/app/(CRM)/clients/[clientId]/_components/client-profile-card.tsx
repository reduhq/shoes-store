...
const ClientProfileCard = ({ client }: { client: Client }) => {
  ...
  // Render stars for rating - Modern version with half stars support
  const renderRatingv2 = (rating: number) => {
        ...
        <div className="flex items-center gap-0.5">
          ...
          {Array(fullStars)
            ...
            .map((_, i) => (
              <div
                key={`full-${i}`}
                className="relative transition-all duration-300 hover:scale-110 text-yellow-400"
              >
                <Star className="h-5 w-5 fill-yellow-400 text-yellow-400 drop-shadow-sm transition-all duration-300" />
                <div className="absolute inset-0 animate-pulse">
                  <Star className="h-5 w-5 fill-yellow-300/30 text-yellow-300/30" />
                </div>
              </div>
            ))}
          ...
          {hasHalfStar && (
            <div className="relative transition-all duration-300 hover:scale-110">
              ...
              <Star className="h-5 w-5 fill-gray-200 text-gray-200 transition-all duration-300" />
          ...
          {Array(emptyStars)
            ...
            .map((_, i) => (
              <div
                key={`empty-${i}`}
                className="relative transition-all duration-300 hover:scale-110 text-gray-200"
              >
                <Star className="h-5 w-5 fill-gray-200 text-gray-200 hover:text-gray-300 transition-all duration-300" />
              </div>
            ))}
        </div>
  ...
  return (
    <Card className="md:col-span-6 lg:col-span-6 xl:col-span-6 2xl:col-span-6 shadow-lg border-0 bg-white/95 backdrop-blur-sm">
      <CardHeader className="text-center pb-3 sm:pb-4">
        ...
        {/* Rating Section - Enhanced with Half Stars */}
        <div className="mt-4 mb-3">
          <div className="text-center">
            <div className="inline-flex items-center justify-center p-4 bg-gradient-to-br from-yellow-50 via-amber-50 to-orange-50 rounded-2xl border border-yellow-100/60 shadow-sm hover:shadow-md transition-all duration-300">
